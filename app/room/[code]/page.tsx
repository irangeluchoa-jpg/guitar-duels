...
const LANE_OPTS = [
  { n:4 as 4|5|6, label:"Fácil",   keys:"A S D J",     color:"#3b82f6", grad:"linear-gradient(135deg,#1d4ed8,#3b82f6)" },
  { n:5 as 4|5|6, label:"Normal",  keys:"A S D J K",   color:"#22c55e", grad:"linear-gradient(135deg,#15803d,#22c55e)" },
  { n:6 as 4|5|6, label:"Difícil", keys:"A S D J K L", color:"#e11d48", grad:"linear-gradient(135deg,#991b1b,#e11d48)" },
]
    ...
    <div className="flex items-end gap-[2px] h-4">
      {[0,1,2,3,4].map(i => (
        <div key={i} className="w-[3px] rounded-full" style={{
          background:color, height:`${40+Math.sin(i*1.3)*30}%`,
...
export default function RoomPage() {
  ...
  useEffect(()=>{
    ...
    const onPlayerLeft = ({ room: r }: { playerId:string; room:RoomData }) => setRoom(r)
  ...
  function handleSetSong(songId: string) {
    setRoom(prev=>prev?{...prev,songId}:prev)
    socketRef.current.emit("set-song",{code,songId})
  }
  ...
  async function handleLeave() {
    const pid=sessionStorage.getItem("playerId")
    if(pid) socketRef.current.emit("leave-room",{code,playerId:pid})
    router.push("/lobby")
  }
          ...
          {filtered.map((song,i)=>{
            ...
            return (
              <button key={song.id}
                ...
                style={{'--dc':songDc,borderLeftColor:isActive?songDc:"transparent",background:isActive?`${songDc}10`:"transparent"} as any}>
                ...
                {isActive&&room.songId===song.id&&<div className="flex-shrink-0"><Waveform color={songDc}/></div>}
              </button>
            )
          })}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="relative z-10 flex items-center justify-between px-6 py-3.5 flex-shrink-0"
          ...
          <button onClick={()=>{playClickSound(getVol());copyCode()}}
            ...
            {copied?<Check className="w-3.5 h-3.5 text-green-400"/>:<Copy className="w-3 h-3 group-hover:opacity-70 opacity-30 text-white transition-opacity"/>}
          </button>
        </div>
          ...
          {selectedSong?(
            <>
              ...
              <div className="flex-1 min-w-0 pt-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] mb-1" style={{color:"rgba(255,255,255,0.3)"}}>{selectedSong.artist}</p>
                <h1 className="bebas text-4xl leading-none text-white mb-3"
                  style={{letterSpacing:"0.02em",textShadow:`0 0 40px ${dc}66`}}>{selectedSong.name}</h1>
                <div className="flex flex-wrap gap-2 mb-3">
                  {selectedSong.genre&&<span className="text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide"
                    style={{background:"rgba(255,150,60,0.12)",color:"rgba(255,150,60,0.8)",border:"1px solid rgba(255,150,60,0.2)"}}>{selectedSong.genre}</span>}
                  {selectedSong.year&&<span className="text-[10px] font-semibold px-2.5 py-1 rounded-full"
                    style={{background:"rgba(255,255,255,0.05)",color:"rgba(255,255,255,0.35)",border:"1px solid rgba(255,255,255,0.08)"}}>{selectedSong.year}</span>}
                  {selectedSong.songLength>0&&<span className="flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full"
                    style={{background:"rgba(255,255,255,0.05)",color:"rgba(255,255,255,0.35)",border:"1px solid rgba(255,255,255,0.08)"}}>
                    <Clock className="w-3 h-3"/>{fmt(selectedSong.songLength)}</span>}
                  <span className="text-[10px] font-bold px-2.5 py-1 rounded-full"
                    style={{background:`${dc}18`,color:dc,border:`1px solid ${dc}44`}}>{DLBLS[selectedSong.difficulty??0]}</span>
                </div>
        ...
        <div className="relative z-10 px-6 pb-6 flex-shrink-0">
          {isHost?(
            <button onClick={()=>{playClickSound(getVol());handleStart()}}
              ...
              style={{
                background:canStart?"linear-gradient(135deg,#7f1d1d,#dc2626 35%,#ef4444 65%,#b91c1c)":"rgba(255,255,255,0.04)",
                ...
                fontFamily:"'Bebas Neue','Impact',sans-serif",letterSpacing:"0.2em",fontSize:"1.25rem",
              }}>
              {canStart&&<Zap className="w-5 h-5 fill-current opacity-70"/>}
              {!room.songId?"ESCOLHA UMA MÚSICA":room.players?.length<2?"AGUARDANDO OPONENTE...":`INICIAR BATALHA — ${room.players?.length} JOGADORES`}
              {canStart&&<Play className="w-5 h-5 opacity-40"/>}
            </button>
          ):(
            <div className="w-full h-16 rounded-2xl flex items-center justify-center gap-2"
              ...
              <span className="text-sm" style={{color:"rgba(255,255,255,0.3)"}}>
                {room.songId?<>Aguardando <span className="font-bold" style={{color:"rgba(255,255,255,0.55)"}}>anfitrião</span> iniciar...</>:"Aguardando o anfitrião escolher uma música..."}
              </span>
            </div>
          )}
        </div>
